...
from apps.shop.serializers import (
    ProductSerializer,
    CreateProductSerializer, CheckItemOrderSerializer, OrderSerializer)
...
class SellersView(APIView):
    ...
    @extend_schema(
        summary='Подать заявку, чтобы стать продавцом',
        description="""
            Эндпоинт позволяет покупателю
             подать заявку на то, чтобы стать продавцом.
        """,
        tags=tags)
...
class ProductsBySellerView(APIView):
    ...
    @extend_schema(
        ...
        description="""
        Эндпоинт возвращает все продукты продавца.
        Продукты можно фильтровать по названию, размерам или цветам.
        """,
        tags=tags,
    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if not category:
                return Response(
                    data={'message': 'Категория не существует!'}, status=404)
...
class SellerProductView(APIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if not product:
            return Response(
                data={'message': 'Данный слаг не найден!'}, status=404)
        ...
        if serializer.is_valid():
            ...
            if not category:
                return Response(
                    data={"message": 'Категория недоступна!'}, status=404)
            data['category'] = category
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not product:
            return Response(
                data={"message": 'Продукт недоступен!'}, status=404)
        elif product.seller != request.user.seller:
            return Response(data={"message": 'Доступн запрещен'}, status=403)
        product.delete()
        return Response(
            data={"message": 'Продукт успешно удален!'}, status=200)
    ...
    def get(self, request):
        ...
        orders = (
            Order.objects.filter(
                orderitems__product__seller=seller).order_by('-created_at')
        )
